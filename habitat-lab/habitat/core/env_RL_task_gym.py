...
class HabGymWrapper(gym.Wrapper):
    """
    Wraps a Habitat RL Task Env into a format compatible with the standard OpenAI Gym
    interface.
    Currently support discrete actions.
    This wrapper therefore changes the behavior so that:
