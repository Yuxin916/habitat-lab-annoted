...
@registry.register_task_action
class TurnLeftSAction(SimulatorTaskAction):
    def step(self, *args: Any, **kwargs: Any):
        r"""Update ``_metric``, this method is called from ``Env`` on each
        ``step``.
        """
...
@registry.register_task_action
class TurnRightSAction(SimulatorTaskAction):
    def step(self, *args: Any, **kwargs: Any):
        r"""Update ``_metric``, this method is called from ``Env`` on each
        ``step``.
        """
...
class Success(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        # metric之间的measurements之间的依赖关系
        # 比如说要先计算DistanceToGoal这个measurements
        # 然后再计算Success这个measurements
    ...
    def update_metric(
        ...
        # get the distance_to_target from the measurements
...
class SPL(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        for i in range(self._sim.habitat_config.num_agents):
            self._previous_position = self._sim.get_agent_state(i).position
        ...
        # print('Success Weighted Path Length:', self._metric)
...
class TopDownMap(Measure):
    ...
    def __init__(
        self, sim: "HabitatSim", config: "DictConfig", *args: Any,
        **kwargs: Any,
    ):
        self._sim = sim
        self._config = config
...
class DistanceToGoal(Measure):
    ...
    def update_metric(
        ...
        for i in range(self._sim.habitat_config.num_agents):
            ...
            # if (self._previous_position[i] is None or
            #     # 当前位置和上一次的位置非常接近
            #     not np.allclose(self._previous_position[i],
            #                     current_position,
            #                     atol=1e-4)
            # ):
...
class DistanceToGoalReward_Diff(Measure):
    ...
    def update_metric(
        ...
        for i in range(self._sim.habitat_config.num_agents):
            # 更新reward
            # 距离目标的距离减去上一次的距离 - 更近 +++ 更远 ---
            self._metric.append(
                -(distance_to_target[i] - self._previous_distance[i]))
            self._previous_distance[i] = distance_to_target[i]
    ...
    def overwrite_sim_config(self, config: Any, episode: Episode) -> Any:
        with read_write(config):
            ...
            if (
                ...
                for i in range(config.simulator.num_agents):
                    if i == 0:
                        # 调用第i个agent的配置
                        ...
                        source_rotation = [0.0, float(np.sin(angle / 2)), 0,
