...
class ObjectNavSigLipNet(Net):
    ...
    def __init__(
        ...
        if backbone.startswith("SigLip"):
            with torch.no_grad():
                self.visual_encoder = VisonTowerEncoder(
                    use_obs_space,
                )
    ...
    def forward(
        ...
        if not self.is_blind:
            ...
            visual_feats = self.test(visual_feats.unsqueeze(1)).view(visual_feats.size(0), -1)
...
class VisonTowerEncoder(nn.Module):
    def __init__(
        self,
        observation_space: spaces.Dict,
        model_name: str = 'spatial_bot_test/siglip',
    ):
        super().__init__()
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        processed_images = self.siglip_preprocess(rgb_pil_images + depth_pil_images,
        ...
        with torch.no_grad():
            ...
            stacked_image = torch.stack([rgb, depth], dim=1).view(n_env, -1, 1152)
